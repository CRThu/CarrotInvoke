...
namespace CarrotInvokeTest
{
    ...
    {
    public:
        TEST_CLASS_INITIALIZE(GlobalSetup)
        {
        ...
        {
            function_info_t* func = find_func("print_hello");
            Assert::IsNotNull(func);
            Assert::AreEqual("print_hello", func->name);

            func = find_func("print_add");
            Assert::IsNotNull(func);
            Assert::AreEqual("print_add", func->name);

            func = find_func("nonexistent_func");
            Assert::IsNull(func);
        }
        ...
        TEST_METHOD(TestInvokeBasic)
        {
            ...
            invoke("print_dec", (int64_t)12345);

            // Test function with HEX64 arg
            invoke("print_hex", (uint64_t)0xABCDEF);

            // Test function with STRING arg
            invoke("print_string", "test string");
        }
        ...
        TEST_METHOD(TestInvokeWithReturn)
        {
            ...
            invoke("print_add", (int64_t)100, (int64_t)200);
        }
        ...
        TEST_METHOD(TestInvokeByPool)
        {
            ...
            function_info_t* func = find_func("print_args");
            Assert::IsNotNull(func);
